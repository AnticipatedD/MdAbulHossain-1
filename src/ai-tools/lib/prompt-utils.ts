...
export function getAvailableEditorTypes(promptDir: string): string[] {
  ...
  try {
    ...
    for (const file of promptFiles) {
      if (file.endsWith('.md')) {
        editorTypes.push(path.basename(file, '.md'));
      }
    }
  } catch {
...
export async function callEditor(
  ...
  for (const msg of prompt.messages) {
    msg.content = msg.content
      .replace('{{markdownPrompt}}', markdownPrompt)
      .replace('{{input}}', content)
      .replace(/<!-- IF_WRITE_MODE -->/g, writeMode ? '' : '<!-- REMOVE_START -->')
      .replace(/<!-- ELSE_WRITE_MODE -->/g, writeMode ? '<!-- REMOVE_START -->' : '')
      .replace(/<!-- END_WRITE_MODE -->/g, writeMode ? '' : '<!-- REMOVE_END -->')
      .replace(/<!-- REMOVE_START -->[\s\S]*?<!-- REMOVE_END -->/g, '');
  }
